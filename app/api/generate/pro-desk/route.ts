...
import { checkAiRateLimit, proDeskRateLimitedBody, rateLimitHeaders, userIdFromRequest } from "@/lib/rate-limit";
...
export async function POST(req: Request) {
  try {
    ...
    if (!limit.allowed) {
      return NextResponse.json(proDeskRateLimitedBody(), { status: 200, headers: rateLimitHeaders(limit) });
    }
    const body = (await req.json()) as GenerateBody;
    const desk = await runProDesk(factsToIntake(body));
    return NextResponse.json(desk, { status: 200, headers: rateLimitHeaders(limit) });
  } catch {
