    ...
    <aside
      ...
      className={
        light
          ...
          : "rounded-xl border border-omni-red/50 bg-red-950/40 px-4 py-3 text-sm text-red-100"
      }
    >
      {ETHICS[locale]}
    </aside>
...
export function MarkerCount({ n, locale }: { n: number; locale: Locale }) {
  ...
  return (
    <p className={`text-sm font-semibold ${n ? "text-omni-yellow" : "text-zinc-400"}`}>{label}</p>
  );
}
